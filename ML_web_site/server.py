...
def load_model():
    ...
    try:
        ...
        if not os.path.exists(model_path):
            ...
            model_path = os.path.join(current_dir, "trained_model3.pkl")
...
def load_model_reg():
    ...
    try:
        model_path = "trained_model_log3.pkl"
        
        if not os.path.exists(model_path):
            current_dir = os.path.dirname(os.path.abspath(__file__))
            model_path = os.path.join(current_dir, "trained_model_log3.pkl")
...
def predict():
    try:
        ...
        study_attendance_interaction = study_hours * attendance 
        # Prepare input data - FIXED: Ensure correct shape and type
        input_data = np.array([[study_hours, attendance, practice_tests, study_attendance_interaction ]], dtype=float)
        
        # Make prediction with Linear Regression
        score_prediction = (model.predict(input_data)[0])
        ...
        input_data_log = np.array([[study_hours, attendance, practice_tests,study_attendance_interaction, score_prediction ]], dtype=float)
        pass_prediction = int(logreg.predict(input_data_log)[0])
